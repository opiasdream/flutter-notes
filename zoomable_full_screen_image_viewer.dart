...
/// A widget that allows the user to zoom in and out of an image.
///
/// The widget is used to display an image in a full screen mode.
/// The image can be zoomed in and out using a double tap.
/// The image can be zoomed in and out using a double tap.
class ZoomableFullScreenImageViewer extends StatefulWidget {
  const ZoomableFullScreenImageViewer({
  ...
  @override
  State<ZoomableFullScreenImageViewer> createState() =>
      _ImageZoomFullscreenState();
}

class _ImageZoomFullscreenState extends State<ZoomableFullScreenImageViewer>
    with SingleTickerProviderStateMixin {
  final TransformationController _transformationController =
      TransformationController();
  late AnimationController _animationController;
  //the higher you set the last value the earlier the full screen gets closed
  late double closingTreshold = MediaQuery.of(context).size.height / 5;
          ...
          child: SizedBox(
            ...
            child: InteractiveViewer(
              ...
              child: GestureDetector(
                // need to have both methods, otherwise the zoom will be
                // triggered before the second tap releases the screen
  ...
  Future<void> _onInteractionUpdate(ScaleUpdateDetails details) async {
    // chose 1.05 because maybe the image was not fully zoomed back but
